...
public class ModelStateDataProvider implements DataProvider, ModelStateData, BlockStateDefinitionData {
    ...
    private final Map<Identifier, Supplier<JsonElement>> states = Maps.newHashMap();
    ...
    @Override
    public void addState(Identifier identifier, Supplier<JsonElement> element) {
    ...
    public void run(DataCache cache) throws IOException {
        ...
        states.forEach((identifier, jsonElement) -> {
            Path outputPath = getStateOutput(path, identifier);
        ...
        models.forEach((identifier, jsonElement) -> {
            Path outputPath = getModelOutput(path, identifier);
    ...
    private static Path getModelOutput(Path rootOutput, Identifier id) {
        return rootOutput.resolve("assets/" + id.getNamespace() + "/models/" + id.getPath() + ".json");
    }
    
    private static Path getStateOutput(Path rootOutput, Identifier id) {
        return rootOutput.resolve("assets/" + id.getNamespace() + "/blockstates/" + id.getPath() + ".json");
    }
